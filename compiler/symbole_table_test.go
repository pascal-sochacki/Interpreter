...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.table.FreeSymbols) != len(tt.expectedFreeSymbole) {
			t.Errorf("wrong number of free smbols. got=%d, want=%d", len(tt.table.FreeSymbols), len(tt.expectedFreeSymbole))
			continue
		}

		for i, sym := range tt.expectedFreeSymbole {
